...
import 'package:dart_application/model/response.dart';
import 'package:dart_application/model/user.dart';
import 'package:dart_application/utils/app_utils.dart';
...
class AppAuthController extends ResourceController {
  ...
  Future<Response> signIn(@Bind.body() User user) async {
    ...
    try {
      ...
      if (requestHashPassword == findUser.hashPassword) {
        ...
        throw QueryException.input('He верный пароль', []);
      }
  ...
  Future<Response> signUp(@Bind.body() User user) async {
    if (user.password == null || user.userName == null || user.email == null) {
      return Response.badRequest(
        body:
            ModelResponse(message: "Поля password username email облзательны"),
      );
    }
    ...
    try {
      ...
      return Response.ok(
        ModelResponse(
          ...
          message: "Товен услешно обновлен",
        ),
      );
    } on QueryException catch (e) {
      return Response.serverError(body: ModelResponse(message: e.message));
